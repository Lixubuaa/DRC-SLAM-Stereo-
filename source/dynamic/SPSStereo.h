...
public:
	...
	void compute(const int superpixelTotal,
				 ...
				 std::vector< std::vector<int> >& boundaryLabels,cv::Mat);
...
private:
	...
	void allocateBuffer();
	void freeBuffer();
	void setInputData(const cv::Mat& leftImage, const cv::Mat& rightImage,cv::Mat vdisp);
	void setLabImage(const cv::Mat& leftImage);
	void computeInitialDisparityImage(const cv::Mat& leftImage, const cv::Mat& rightImage,cv::Mat disp);
	...
	//output contour
	std::vector<std::vector<cv::Point2f>> DynamicContour;
