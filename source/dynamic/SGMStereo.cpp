...
{
	double start_time, end_time, t0, t1, t2, t3, t4;
	...
	std::cout << "sgm_algorithms 1st 耗时：" << t0 - start_time << "s" << std::endl;
	std::cout << "sgm_algorithms 2nd 耗时：" << t1 - t0 << "s" << std::endl;
	std::cout << "sgm_algorithms 3rd 耗时：" << t2 - t1 << "s" << std::endl;
	std::cout << "sgm_algorithms 4th 耗时：" << t3 - t2 << "s" << std::endl;

	std::cout << "sgm_algorithms 5th 耗时：" << t4 - t3 << "s" << std::endl;
	std::cout << "sgm_algorithms 6th 耗时：" << end_time - t4 << "s" << std::endl;
	std::cout << "sgm_algorithms 总耗时：" << end_time - start_time << "s" << std::endl;
}
...
void SGMStereo::computeLeftCostImage(const unsigned char* leftGrayscaleImage, const unsigned char* rightGrayscaleImage) {
	...
	std::cout << "computeLeftCostImage 2nd 耗时：" << t1 - t0 << "s" << std::endl;
	std::cout << "computeLeftCostImage 3rd 耗时：" << t2 - t1 << "s" << std::endl;
	std::cout << "computeLeftCostImage 4th 耗时：" << t3 - t2 << "s" << std::endl;
	std::cout << "computeLeftCostImage 5th 耗时：" << t4 - t3 << "s" << std::endl;
}
							 ...
							 unsigned short* costImageRow)
{
	...
	int addRowIndex, addRowAggregatedCostIndex;
	unsigned short* addRowAggregatedCost;
	int subRowAggregatedCostIndex;
	const unsigned short* subRowAggregatedCost;
	const unsigned short* previousCostRow;
	const unsigned char* addPixelwiseCost;
	const unsigned char* subPixelwiseCost;
	__m128i registerAddPixelwiseLow, registerAddPixelwiseHigh, registerSubPixelwiseLow, registerSubPixelwiseHigh, registerAddAggregated, registerCost;
	for (int y = 1; y < height_; ++y) {
		addRowIndex = y + aggregationWindowRadius_;
		addRowAggregatedCostIndex = std::min(addRowIndex, height_ - 1) % (aggregationWindowRadius_ * 2 + 2);
		addRowAggregatedCost = rowAggregatedCost_ + width_*disparityTotal_*addRowAggregatedCostIndex;
		...
		if (addRowIndex < height_) {
			...
			// x = 1...width-1
			subRowAggregatedCostIndex = std::max(y - aggregationWindowRadius_ - 1, 0) % (aggregationWindowRadius_ * 2 + 2);
			subRowAggregatedCost = rowAggregatedCost_ + width_*disparityTotal_*subRowAggregatedCostIndex;
			previousCostRow = costImageRow - widthStepCost;
			for (int x = 1; x < width_; ++x) {
				addPixelwiseCost = pixelwiseCostRow_
					+ std::min((x + aggregationWindowRadius_)*disparityTotal_, (width_ - 1)*disparityTotal_);
				subPixelwiseCost = pixelwiseCostRow_
					+ std::max((x - aggregationWindowRadius_ - 1)*disparityTotal_, 0);

				for (int d = 0; d < disparityTotal_; d += 16) {
					registerAddPixelwiseLow = _mm_load_si128(reinterpret_cast<const __m128i*>(addPixelwiseCost + d));
					registerAddPixelwiseHigh = _mm_unpackhi_epi8(registerAddPixelwiseLow, registerZero);
					registerAddPixelwiseLow = _mm_unpacklo_epi8(registerAddPixelwiseLow, registerZero);
					registerSubPixelwiseLow = _mm_load_si128(reinterpret_cast<const __m128i*>(subPixelwiseCost + d));
					registerSubPixelwiseHigh = _mm_unpackhi_epi8(registerSubPixelwiseLow, registerZero);
					registerSubPixelwiseLow = _mm_unpacklo_epi8(registerSubPixelwiseLow, registerZero);

					// Low
					registerAddAggregated = _mm_load_si128(reinterpret_cast<const __m128i*>(addRowAggregatedCost
					...
					registerCost = _mm_load_si128(reinterpret_cast<const __m128i*>(previousCostRow + disparityTotal_*x + d));
