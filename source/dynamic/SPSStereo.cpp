...
void SPSStereo::compute(const int superpixelTotal,
						...
						std::vector< std::vector<int> >& boundaryLabels,cv::Mat vdisp)
{
...
//	t0 = (double)clock() / CLOCKS_PER_SEC;
	...
	setInputData(leftImage, rightImage, vdisp);
...
//	std::cout << "sps_algorithms 1st 耗时：" << t0 - start_time << "s" << std::endl;
//	std::cout << "sps_algorithms 2nd 耗时：" << t1 - t0 << "s" << std::endl;
//	std::cout << "sps_algorithms 3rd 耗时：" << t2 - t1 << "s" << std::endl;
//	std::cout << "sps_algorithms 4th 耗时：" << t3 - t2 << "s" << std::endl;

//	std::cout << "sps_algorithms 5th 耗时：" << t4 - t3 << "s" << std::endl;
//	std::cout << "sps_algorithms 6th 耗时：" << end_time - t4 << "s" << std::endl;
	std::cout << "sps_algorithms 总耗时：" << end_time - start_time << "s" << std::endl;
	freeBuffer();
}
...
void SPSStereo::setInputData(const cv::Mat& leftImage, const cv::Mat & rightImage,cv::Mat vdisp) {
	//double start_time, end_time, t0;
//	start_time = (double)clock() / CLOCKS_PER_SEC;
	
	setLabImage(leftImage);
//	t0 = (double)clock() / CLOCKS_PER_SEC;
	computeInitialDisparityImage(leftImage, rightImage, vdisp);
//	end_time = (double)clock() / CLOCKS_PER_SEC;
	//std::cout << "set函数耗时 1st：" << t0 - start_time << "s" << std::endl;
	//std::cout << "set函数耗时 2nd：" << end_time - t0 << "s" << std::endl;
}
...
void SPSStereo::computeInitialDisparityImage(const cv::Mat& leftImage, const cv::Mat & rightImage,cv::Mat sdisp) {
	//用opencv的sgbm计算的视差图
	//cv::Mat disp,vdisp;
	//cv::StereoSGBM sgbm;
	//sgbm.preFilterCap = 64; //预处理滤波器的截断值，预处理的输出值仅保留[-preFilterCap, preFilterCap]范围内的值，参数范围
	//sgbm.SADWindowSize = 5;

	//int cn = leftImage.channels();

	////控制视差变化平滑性的参数。P1、P2的值越大，视差越平滑。P1是相邻像素点视差增/减 1 时的惩罚系数；P2是相邻像素点视差变化值大于1时的惩罚系数。P2必须大于P1
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			initialDisparityImage_[width_*y + x] = static_cast<float>(sdisp.at<uchar>(y, x));
	...
	//SGMStereo sgm;
	//sgm.compute(leftImage, rightImage, initialDisparityImage_);//SPS算法计算的视差图
	//cv::Mat disp;
	/*std::fstream initialDisp;
	
	initialDisp.open("initialdisp2.txt", std::ios::out | std::ios::trunc);
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			initialDisp << initialDisparityImage_[width_*y + x]<<std::endl;
		}
	}
	initialDisp.close();*/
	
	//std::cout << "sgm compute输出："<<initialDisparityImage_ << std::endl;
}
...
void SPSStereo::makeGridSegment(const int superpixelTotal) {
	...
	for (int y = 0; y < height_; ++y) {
		...
		for (int x = 0; x < width_; ++x) {
			...
			segments_[segmentIndex].addPixel(x, y, inputLabImage_[width_ * 3 * y + 3 * x], inputLabImage_[width_ * 3 * y + 3 * x + 1], inputLabImage_[width_ * 3 * y + 3 * x + 2]); //position,L,a,b
		}
	}
...
void SPSStereo::assignLabel() {
	...
	while (!boundaryPixelIndices.empty()) {
		...
		int pixelY = pixelIndex/width_;//边界像素
		boundaryFlagImage_[width_*pixelY + pixelX] = 0;//边界像素的flag
...
bool SPSStereo::isUnchangeable(const int x, const int y) const {
	...
	int outCount = 0;
	for (int neighborIndex = 0; neighborIndex < eightNeighborTotal; ++neighborIndex) {//8
...
void SPSStereo::interpolateDisparityImage(float* interpolatedDisparityImage) const {
	...
	//std::cout << "^^^^^^^^^^^^^^^^^^^^^" << std::endl;
	//cv::Mat _disp;
	//_disp.create(height_, width_,CV_8UC1);
	//for (int y = 0; y < height_; ++y) {
	//	for (int x = 0; x < width_; ++x) {
	//		//std::cout << interpolatedDisparityImage[width_*y + x]<<std::endl;
	//		_disp.at<uchar>(y, x) = interpolatedDisparityImage[width_*y + x];
	//	}
	//}
	//cv::imshow("_disp", _disp);
	//cv::waitKey(3);
