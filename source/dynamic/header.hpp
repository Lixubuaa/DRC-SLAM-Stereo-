...
enum UNDISTORT_MODE{
	UNDISTORT_PTS, UNDISTORT_IMG, _NONE
};
...
public:
	...
	MOD(string capture, string& in, bool usefisheye = true, UNDISTORT_MODE m = _NONE, double r_thre = 0.2, double thre_2epi = 5, double h_d = 0.167, double w_d = 0.167) :
	...
	vector<Point2f> process(Mat preframe, Mat frame ,Mat disp);
	...
	bool checkROI(Point2f);
private:

	fstream Epipolar_line;
    //Input video
    VideoCapture cap;
